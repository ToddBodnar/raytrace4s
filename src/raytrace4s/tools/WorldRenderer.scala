...
class WorldRenderer(width: Int, height: Int, world: World, sqrtSubSamples: Int, maxBounces: Int) {
  ...
  def render(x: Int, y: Int): Color = {
    ...
    def render(xCt: Int, yCt: Int, acc: Vector3d): Vector3d = {
      if (yCt >= sqrtSubSamples) {
        ...
        render(xCt + 1, yCt, acc + (world.fire(new Ray(origin, (lowerLeft + horizontal * ((x + 1.0 * xCt / sqrtSubSamples - 0.5) * 1.0 / width) + vertical * ((y + 1.0 * yCt / sqrtSubSamples - 0.5) * 1.0 / height)).unit), maxBounces).vector / sqrtSubSamples / sqrtSubSamples))
      }
    }
